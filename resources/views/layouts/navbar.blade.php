...
<nav class="navbar navbar-expand-lg custom-navbar sticky-top py-2">
    <div class="container">
        <!-- Logo / Brand -->
        <a class="navbar-brand navbar-brand-custom d-flex align-items-center gap-2" href="{{ route('dashboard') }}">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0 ms-lg-3 gap-1">
                ...
                {{-- Menampilkan menu Users hanya jika route tersedia --}}
                @if (Route::has('admin.users'))
                <li class="nav-item">
                    <a class="nav-link nav-link-custom {{ Request::is('admin/users*') ? 'active' : '' }}" href="{{ route('admin.users') }}">
                ...
                <li class="nav-item">
                    <a class="nav-link nav-link-custom {{ Request::is('produk*') ? 'active' : '' }}" href="{{ route('produk.index') }}">
                        <i class="bi bi-box-seam me-1"></i> Produk
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link nav-link-custom {{ Request::is('penjualan*') ? 'active' : '' }}" href="{{ route('penjualan.index') }}">
                        <i class="bi bi-receipt me-1"></i> Penjualan
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link nav-link-custom {{ Request::is('about') ? 'active' : '' }}" href="{{ route('about') }}">
            ...
            <div class="d-flex align-items-center gap-3 pt-2 pt-lg-0">
                @auth
                    <div class="d-none d-lg-block text-end">
                        <div class="fw-bold text-white small" style="line-height: 1.2;">{{ Auth::user()->name }}</div>
                        <small class="text-info" style="font-size: 0.75rem;">{{ Auth::user()->role->name ?? (is_string(Auth::user()->role) ? Auth::user()->role : 'Kasir') }}</small>
                    </div>
                @endauth
