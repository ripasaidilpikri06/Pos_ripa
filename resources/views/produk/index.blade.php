...
<div class="dashboard-container">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 pb-2 border-bottom border-secondary border-opacity-25">
        ...
        <div class="mt-3 mt-md-0">
            ...
            <a href="{{ route('produk.create') }}" class="btn btn-primary px-4 py-2 rounded-pill fw-semibold shadow-sm">
                ➕ Create Produk
            </a>
            @endcan
        </div>
            ...
            <div class="col-12 col-sm-6 col-md-4 col-lg-3">
                <div class="card product-card h-100">
                    <div class="product-img-wrapper">
                        ...
                        <span class="position-absolute top-0 end-0 m-3 badge rounded-pill {{ $product->stok > 0 ? 'bg-success' : 'bg-danger' }} shadow-sm">
                            Stok: {{ $product->stok }}
                        </span>
                    </div>
                    ...
                    <div class="card-body d-flex flex-column justify-content-between p-4">
                        <div>
                            <div class="d-flex align-items-center mb-2">
                                <span class="badge badge-soft-light me-2">#{{ $products->firstItem() + $loop->index }}</span>
                                <small class="text-secondary text-truncate">👤 {{ $product->user->name ?? '-' }}</small>
                            </div>
                            <h5 class="card-title fw-bold text-white text-truncate mb-3" title="{{ $product->nama }}">
                                {{ $product->nama }}
                            </h5>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <span class="fw-bold text-info fs-6">
                                        Rp {{ number_format($harga_jual_diskon, 0, ',', '.') }}
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="pt-3 border-top border-secondary border-opacity-25 mt-2">
                            <div class="d-flex gap-2">
                                @can('update', $product)
                                <a href="{{ route('produk.edit', $product->id) }}" class="btn btn-primary btn-sm w-100 rounded-pill fw-semibold">
                                    ✏️ Edit
                                </a>
                                @endcan

                                @can('delete', $product)
                                <form action="{{ route('produk.destroy', $product->id) }}" method="POST" class="w-100">
                                    @csrf
                                    @method('DELETE')
